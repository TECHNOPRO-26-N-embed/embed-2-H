...
| 作品タイトル | 変音（ミューチェン） |
| 一言説明（何ができるガジェットか） | |
| 一言説明（何ができるガジェットか） | 音を変更したり音程や音量を調節するガジェット |
| 使用する共通部品（グループ共通） | |
| 使用する共通部品（グループ共通） | Buzzer |
| 追加部品（個人・主なもの） | |
| 追加部品（個人・主なもの） | Rotary Encoder、ボタン、LED、抵抗、ジャンパワイヤ |
| 必須機能の数（3-1の件数） | 　件 |
| 必須機能の数（3-1の件数） | 3件 |
...
| グループ共通部品（名称） | Buzzer |
| 共通部品が担う役割 | 音の出力（音色・音量・音程の変化を体感できる） |
| 接続するArduinoピン（予定） | D9（予定） |
| 個人の設計と共通部品が協調する箇所 | エンコーダやボタン操作でBuzzerの音色・音量・音程を制御する |
| グループ内で統一すべき仕様 | Buzzerのピン番号、音量・音程の段階数 |
...
【入力】                 【Arduino UNO R3】            【出力】
Rotary Encoder ─────→┐
ボタン         ─────→┤   音制御・状態管理         ├──→ Buzzer（音出力）
                     └──────────────────────┘──→ LED（状態/エラー表示）
```
補足説明：
ロータリーエンコーダで音程・音量・音色を選択・変更し、ボタンで音色切替や決定操作を行う。LEDは現在の設定やエラー時の状態を表示する。
...
| F01 | 音の変更 | `doFeature1()` | 音色を切り替える | なし | なし | loop()内 |
| F02 | 音量変更 | `doFeature2()` | 音量を段階的に調整 | なし | なし | loop()内 |
| F03 | 音程変更 | `doFeature3()` | 音程を変更する | なし | なし | loop()内 |
| A01 | 音パターン記憶/呼出 | `doOptional1()` | 音パターンを記憶・呼出 | なし | なし | loop()内 |
| A02 | LED状態表示 | `doOptional2()` | LEDで状態/エラー表示 | なし | なし | loop()内 |
|  |  |  |  |  |  |  |
|  |  |  |  |  |  |  |
